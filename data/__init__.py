...
_datasets = {"wikitext": build_data_from_wikitext,
             "test": build_data_from_random
            }

def build_data(**args):
    if hasattr(gpc.config, "dataset"):
        assert gpc.config.dataset in _datasets.keys() , f'Invalid dataset name. dataset should be in {_datasets.keys()} or use default wikitext'
